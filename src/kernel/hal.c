...
void hal_init(void) {
    debug_printf("[DEBUG] Starting hardware abstraction layer initialization...\n");
	...
	debug_printf("[DEBUG]\t- System timer initialized with %dHz frequency.\n", system_timer_frequency);
    ...
    // Enable IRQ's
	sti();
    debug_printf("[DEBUG]\t- Enabled IRQ's.\n");

    debug_printf("[DEBUG] Finished initializing hardware abstraction layer.\n");
}